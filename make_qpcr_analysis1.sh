...
#
# Extracts and represent the data from QuantumStudio 3. This execution is necessary prior to any further analysis.
# 
# Automatic folders are created at the analysis folder for specific experiments. 
# 
# 
...
#
# OUTPUT:
# 	Data file containing CT values stats, Q values stats and original data. The file contains the basename of the original file suffixed with 
#       "_Results_". stats focus on the analysis of mean values in a parametric fashion.
#
# 	PDF representing the mean, SEM (box) and CI (whiskers) of the indicated categories
#
...
declare -r CONFIGFILE_NAME="config"
...
R --vanilla --slave --file="${WD}/${ANFLD}/make_qpcr_analysis1.R" --args "${WD}" "${DATAFLD}" "${ANFLD}" "${DATAEXPFLD}" "${DATAEXPFILE}" "${PLATERANKS}" "${CATEGS}" "${CATEGS_NMS}" "${STDCURVE_POS}" "${SKIPLINES}" "${ALPHA}" "${WGRAPH}" "${WH}" "${DELOUT}" "${DELHIGHSD}" "${EXCLUDING_FIELDS}" "${FACTORING_FIELDS}"
